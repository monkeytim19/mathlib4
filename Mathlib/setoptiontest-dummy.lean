...
-- XXX: more tests where the old set_option linter doesn't work would be nice
...
#guard_msgs in
set_option linter.setOption true in
set_option trace.profiler.output "foo" -- not found by python script!
...
lemma tactic : True := by
  set_option pp.all true in -- IS found by script
  trivial
...
lemma tactic2 : True := by
  set_option pp.raw.maxDepth 32 in -- IS found
  trivial
...
lemma tactic3 : True := by
  set_option pp.all false in -- IS found
  trivial
...
lemma tactic4 : True := by
  set_option trace.profiler.output "foo" in -- IS NOT found
  trivial
